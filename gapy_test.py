...
class ManagementClientTest(unittest.TestCase):
  ...
  def mock_list(self, name):
    ...
    getattr(self.service.management(), name)().list.return_value.execute.return_value = data

  def assert_list_called(self, name, **kwargs):
    getattr(self.service.management(), name)().list.assert_called_once_with(**kwargs)
  ...
  def test_profiles(self):
    ...
    self.assert_list_called("profiles", accountId="26179049", webPropertyId="UA-26179049-1")
    self.assertEqual(profiles.username, "673591833363@developer.gserviceaccount.com")
...
class QueryClientTest(unittest.TestCase):
  ...
  def mock_get(self, name):
    data = fixture("%s.json" % name)
    self.service.data.return_value.ga.return_value.get.return_value.execute.return_value = data

  def assert_get_called(self, **kwargs):
    self.service.data.return_value.ga.return_value.get.assert_called_once_with(**kwargs)

  def get_call_args_list(self):
    return self.service.data.return_value.ga.return_value.get.call_args_list
